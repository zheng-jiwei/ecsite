...
function show_login_page(){
	...
	window.location.href = "/customer/login.html?redirect=" + currentURL;
}
...
function set_left_product_list(){
    ...
    get_product_by_product_id("4283175665769", function(result){
        ...
        list.push("<div><a href=\"/products/product.html?pid=" + pid +"\"><img src=\"" + url + "\" height='160px' width='160px'></a></div>");
	...
	$("#page-header").load("/page_header.html", function(){
		get_normal_collection(9, function(result){
			...
			for(var i=0; i<8; i++) {
				...
				td_array.push("<td><a href=\"/collections/collection_one.html?cid="+ cid +"\"><img src=\"" + obj[i].node.image.src + "\" width=\"285px\" height=\"100px\"></a></td>");
			}
			...
			if( document.getElementById("normal_collections")){
				...
				if(document.getElementById("sales_collection")){
					...
					document.getElementById("sales_collection").innerHTML = "<div><a href=\"/collections/collection_one.html?cid="+cid +"\"><img src=\"" + obj[8].node.image.src + "\"></a></div>";
				}
...
if($("#page-left").length > 0){
	$("#page-left").load("/page_left.html", function(){
